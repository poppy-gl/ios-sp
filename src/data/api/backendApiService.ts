...
type BackendVideoListOptions = BackendApiOptions & {
  category?: string;
  cursor?: string;
  page?: number;
  pageSize?: number;
...
export type BackendHealthResponse = { ok?: boolean; updatedAt?: string };

export type BackendVideoPageResponse = {
  hasMore: boolean;
  items: VideoItem[];
  nextCursor?: string;
  page?: number;
...
const VIDEO_LIST_API_TIMEOUT_MS = 20_000;
const createStringSet = <T extends string>(value: string) => new Set(value.split(' ') as T[]);
const validSourceTypes = createStringSet<VideoSourceType>(
  'mp4 m3u8 hls dash mov m4v mkv webm avi flv ts mpeg mpg 3gp rmvb mp3 aac wav m4a unknown webview unsupported',
);
const directSourceTypes = new Set<VideoSourceType>(['mp4', 'm3u8', 'hls', 'mov', 'm4v']);
const validFormats = createStringSet<VideoFormat>(
  'mp4 m3u8 hls dash mov m4v mkv webm avi flv ts mpeg mpg 3gp rmvb mp3 aac wav m4a unknown',
);
const validCodecs = createStringSet<VideoCodec>('h264 h265 hevc vp8 vp9 av1 aac unknown');
...
const parseFallbackBaseUrls = (...values: (string | undefined)[]) => {
...
export const getBackendApiConfig = (): BackendApiConfig => {
  ...
  const fallbackBaseUrls = parseFallbackBaseUrls(
    getEnv('EXPO_PUBLIC_VIDEO_API_FALLBACK_BASE_URL'),
    getEnv('EXPO_PUBLIC_VIDEO_API_FALLBACK_BASE_URLS'),
    getEnv('API_FALLBACK_BASE_URL'),
    getExtra('apiFallbackBaseUrl'),
    getExtra('apiFallbackBaseUrls'),
  ...
  return {
    baseUrl,
    configured: Boolean(baseUrl || fallbackBaseUrls.length > 0),
    fallbackBaseUrls,
...
const buildUrl = (
  ...
  return `${baseUrl}${pathname}${queryText ? `?${queryText}` : ''}`;
};

const getPayloadItemCount = (payload: unknown) => extractItems(payload).length;

const getBackendBaseUrlCandidates = (config: BackendApiConfig) => {
  const candidates = [config.baseUrl, ...config.fallbackBaseUrls].filter((value): value is string =>
    Boolean(value),
  );
  const seen = new Set<string>();
...
const isRetryableBackendError = (error: unknown) => {
  if (error instanceof BackendApiError) {
    return error.status === undefined || [502, 503, 504].includes(error.status);
  }
...
const requestBackend = async <T>(
  ...
  const baseUrlCandidates = getBackendBaseUrlCandidates(config);
  ...
  for (const [index, baseUrl] of baseUrlCandidates.entries()) {
    ...
    try {
      if (index > 0) {
        console.info('[backendApiService] retrying fallback', {
          path,
          url,
        });
      }
      ...
      console.info('[backendApiService] response', {
      ...
      console.info('[backendApiService] payload', {
        items: getPayloadItemCount(payload),
        path,
      });

      return payload;
    } catch (error) {
      const message = controller.timedOut()
        ? `Backend API request timed out after ${controller.timeoutMs}ms.`
        : error instanceof Error
          ? error.message
          : String(error);
      console.warn('[backendApiService] request failed', {
        error: message,
        path,
        url,
      });

      lastError = controller.timedOut() ? new BackendApiError(message) : error;

      if (options?.signal?.aborted || index === baseUrlCandidates.length - 1) {
        break;
      }
...
export const fetchBackendVideos = async (options?: BackendVideoListOptions): Promise<VideoItem[]> =>
  (await fetchBackendVideoPage(options)).items;

export const fetchBackendVideoPage = async (
  options?: BackendVideoListOptions,
): Promise<BackendVideoPageResponse> => {
  const payload = await requestBackend<unknown>('/api/videos', {
    ...
    signal: options?.signal,
    timeoutMs: options?.timeoutMs ?? VIDEO_LIST_API_TIMEOUT_MS,
  });

  const parsedItems = parseVideoItems(payload);
  ...
  const hasMore =
    typeof record.hasMore === 'boolean'
      ? record.hasMore
      : parsedItems.length >= Math.max(Number(options?.pageSize ?? parsedItems.length), 1);
...
export const fetchBackendHealth = async (
  ...
  requestBackend<BackendHealthResponse>('/api/health', {
    signal: options?.signal,
