...
const EPISODE_RESOLVE_TIMEOUT_MS = 22_000;
...
const getPlaybackDecision = (video: PlayerVideo | null): PlaybackDecision => {
  ...
  const playbackAvailability = video ? getPlaybackAvailability(video) : 'unplayable';
  ...
  if (playbackAvailability === 'lazy') {
    return {
      canPlayInApp: false,
      label: 'Resolving',
      reason: isNativeVideoAvailable
        ? '正在通过后端解析播放地址，不会打开网页。'
        : text.unsupportedPlaybackHint,
      reasonCode: 'lazy-resolve-required',
      supportLevel: 'unsupported',
    };
  }
...
const needsFullVideoDetail = (video?: PlayerVideo | null) => {
  if (!video) {
    return true;
  }

  const lineCount = video.playLines?.length ?? 0;
  const episodeCount =
    video.playLines?.reduce((total, line) => total + line.episodes.length, 0) ?? 0;

  return lineCount === 0 || episodeCount === 0 || (lineCount <= 1 && episodeCount <= 1);
};
...
export default function PlayerScreen() {
  ...
  const playbackReasonCode = playbackDecision.canPlayInApp
    ? undefined
    : playbackDecision.reasonCode;
  const shouldShowResolveLoading =
    isResolvingEpisode || (playbackReasonCode === 'lazy-resolve-required' && !playbackError);
  const unsupportedDetailText =
    playbackError || !playbackReasonCode || playbackReasonCode === 'lazy-resolve-required'
      ? text.unsupportedPlaybackHint
      : `${text.supportReason}: ${playbackReasonCode}`;
  const tags = video?.tags?.filter(Boolean) ?? [];
  const authorLabel = getPublicProviderLabel(video?.author, text.authorFallback);
  const providerLabel = getPublicProviderLabel(video?.provider);
  ...
  useEffect(() => {
    ...
    const loadVideo = async () => {
      try {
        const relatedVideosPromise = getAllVideos().catch(() => []);
        const cachedOrDetailVideo = (await getVideoById(id)) as PlayerVideo | undefined;
        const freshVideo = needsFullVideoDetail(cachedOrDetailVideo)
          ? ((await getVideoById(id, { bypassCache: true })) as PlayerVideo | undefined)
          : undefined;
        const nextVideo = freshVideo ?? cachedOrDetailVideo;
        ...
        void relatedVideosPromise.then((allVideos) => {
          if (mounted) {
      ...
      for (const candidate of candidates) {
        ...
        try {
          const result = await resolveEpisodeMediaUrl(
            ...
            { signal: controller.signal, timeoutMs: EPISODE_RESOLVE_TIMEOUT_MS },
          );
          ...
          {playbackDecision.canPlayInApp ? (
            ...
            <View style={styles.unsupportedPlayer}>
              ...
              <Text style={styles.unsupportedTitle}>
                {playbackError || playbackDecision.reason}
              </Text>
              <Text style={styles.unsupportedText}>{unsupportedDetailText}</Text>
            </View>
          )}
        </View>
              ...
              <View style={styles.authorBlock}>
                ...
                <View style={styles.authorContent}>
                  ...
                  <Text style={styles.providerText} numberOfLines={1}>
                    {providerLabel} / {video.category ?? text.categoryFallback}
                  </Text>
                </View>
              </View>
                  ...
                  >
                    ...
                    <View style={styles.relatedContent}>
                      ...
                      <Text numberOfLines={1} style={styles.relatedMeta}>
                        {getPublicProviderLabel(item.author, text.authorFallback)} /{' '}
